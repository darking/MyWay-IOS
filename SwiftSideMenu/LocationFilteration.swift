...
class LocationFilteration{
    ...
    func filterLocations(allReports: NSMutableArray, currentLocation:CLLocation) ->NSArray{
...
//        }
        ...
        for (var i = 0 ; i < allReports.count ; i++) {
            ...
            var months = getInfo.AddToArray().valueForKey("month")
            ...
            var defaultMonth = NSUserDefaults.standardUserDefaults().valueForKey("month") as! Int
            ...
            var indexMonth = months?.objectAtIndex(i) as! Int
            var indexYear=years?.objectAtIndex(i) as! Int
            
            if ((defualtTime>=indexTime) && (defualtDay==indexDay) && (defaultMonth==indexMonth) && (defualtTYear==indexYear) )
            {
                println("defualt \(defualtTime)")
                println("index time \(indexTime)")
                println("defualt \(defualtDay)")
                println("index time \(indexDay)")
...
//                && (defualtDay==indexDay) && (defaultMonth==indexMonth) && (defualtTYear==indexYear) )
//            {
//                nearReports.addObject(currentReport);
//                
//            }
